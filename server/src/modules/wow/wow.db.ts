...
class WowDB {
  ...
  private getSummary = async ({ access_token, region }: { access_token: string; region: string }) => {
    const result = await axios.get<Summary[]>(`https://${region}.api.blizzard.com/profile/user/wow?namespace=profile-${region}&locale=en_US&access_token=${access_token}`);
    return result;
  };

  private getCharacterMedia = async ({ region, char_name, realm_slug, access_token }: GetDbInfo) => {
  ...
  private getProtectedCharacterData = async ({ region, realm_id, char_id, access_token }: { region: string; realm_id: string; char_id: string; access_token: string }) => {
  ...
  private getCharacterSummary = async ({ region, char_name, realm_slug, access_token }: GetDbInfo) => {
  ...
  private getEquipmentMedia = async ({ equipment, access_token }: { equipment: EquippedItemsEntity[]; access_token: string }) => {
    ...
    for (let i = 0; i < equipment.length; i++) {
      ...
      mediaResult[itemName] = itemMediaRequest.data.assets ? itemMediaRequest.data.assets[0].value : '';
    }

    return mediaResult;
  };

  private getCharacterEquipment = async ({ region, realm_slug, char_name, access_token }: GetDbInfo) => {
  ...
  private getCharacterStatistics = async ({ region, realm_slug, char_name, access_token }: GetDbInfo) => {
  ...
  private getCharacterDungeons = async ({ region, realm_slug, char_name, access_token }: GetDbInfo) => {
  ...
  private getCharacterQuests = async ({ region, realm_slug, char_name, access_token }: GetDbInfo) => {

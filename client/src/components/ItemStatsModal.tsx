...
const ItemStatsModal = ({ stats, image }: { stats: EquippedItemsEntity | undefined; image?: string }) => {
  ...
  const handleCloseModal = () => {
    const lastFocusedElement = document.getElementById(stats!.item.id.toString());
  ...
  return (
    <div className="fixed left-0 top-0 w-full h-full flex sm:items-center sm:justify-center z-10 ">
      ...
      <div className="absolute left-0 right-0 w-full h-full bg-black bg-opacity-70 z-20 " onClick={handleCloseModal}></div>
      {stats && (
        <div
        ...
        >
          <div className="flex justify-between items-center mb-4 w-full">
            ...
            <button className="relative flex min-w-[25px] min-h-[25px] p-2" onClick={handleCloseModal} aria-label={'close item modal'} autoFocus ref={firstFocusableElement}>
          ...
          {stats?.set?.items?.map((item) => {
            return <p className={`${item.is_equipped ? 'text-white' : 'text-gray-500'} ml-4 text-md`}>{item.item.name}</p>;
          })}

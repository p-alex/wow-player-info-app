...
const ItemSlot = ({ stats, isWeapon, image }: { stats: EquippedItemsEntity | undefined; isWeapon?: boolean; image?: string }) => {
  ...
  const handleStatsPos = (e: React.MouseEvent<HTMLDivElement | HTMLButtonElement, MouseEvent>) => {
  ...
  return (
    <>
      {selectedItemStats && selectedItemStats.item.id === stats?.item.id && <ItemStatsModal stats={stats} image={image} />}
      <button
        className={`relative w-[50px] h-[50px] bg-slate-900 border ${itemQuality ? QUALITY_COLORS[itemQuality]?.border : 'border-slate-700'} bg-contain bg-no-repeat bg-center`}
      ...
      >
        {isStatsBubbleActive && <ItemStatsBubble stats={stats} isWeapon={isWeapon} statsPos={statsPos} />}
      </button>
    </>
  );
};
